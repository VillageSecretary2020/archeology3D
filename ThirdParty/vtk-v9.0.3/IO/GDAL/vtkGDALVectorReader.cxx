...
public:
  ...
  {
    *pd = vtkPolyData::New();
    vtkCellData* pcd=(*pd)->GetCellData(); 
	//std::cout << "SetupPolyData{" << std::endl;
    ...
    for (f = 0; f < numFields; ++f)
    {
      ...
      //std::cout <<"OGRFieldDefn:\t"<< ffdef->GetNameRef()<<std::endl;
    ...
    vtkDoubleArray *sdr= vtkDoubleArray::New();
    sdr->SetNumberOfComponents(1);
	pcd->SetScalars(sdr);
    sdr->FastDelete();//? 
  ...
  #define AUTOCAD_COLOR_NUM 22
  typedef struct st_AUTOCAD_COLOR
  {
    const char* hex_str;
    const int index;
  } AUTOCAD_COLOR;
  
   const AUTOCAD_COLOR autocad_color_table[AUTOCAD_COLOR_NUM] = {
   // https://blog.csdn.net/qq_44792233/article/details/108529699
    {"000000", 0},	// black
	{"ff0000", 1},		// red
	{"ffff00", 2},
	{"00ff00", 3},
   ...
   int get_autocad_color_index(std::string hex)
   {
      int color_index; 
      for (color_index=0; color_index<22; color_index++)
      {
        if (0 == hex.compare(autocad_color_table[color_index].hex_str))
        {
			//std::cout << "index="<<color_index ;
			break;
        }
      }
      if (AUTOCAD_COLOR_NUM == color_index)
        color_index = 7;	//set default color with white
      return color_index;
  }
  bool ReadLayer(OGRLayer* layer, vtkMultiBlockDataSet* mbds)
  {
   ...
   // std::cout << "layer:\t"<<layer->GetName()<<std::endl;
    ...
    //std::cout << "OGRFeatureDefn:\t"<<fdef->GetName()<< "\tnumFields:\t"<<numFields <<std::endl;
    ...
    while ((feat = layer->GetNextFeature()))
    {
      ...
      std::string sstr = feat->GetStyleString();
      //std::cout <<"OGRFeature style:\t"<< sstr << '\t';
      size_t offs = sstr.find("c:#");
      sstr = sstr.substr(offs + 3, 6);
     // std::cout << std::endl;
     // std::cout << pts->GetData()->GetDataTypeAsString() << std::endl;
      int color_index = get_autocad_color_index(sstr);
      vtkIdType nPoly = this->insertGeometryRecursive(geom, pd, pts, lines, verts, color_index);
      if (!nPoly)
      {
        continue;
      }
  ...
  vtkIdType insertGeometryRecursive(
    OGRGeometry* geom, vtkPolyData* pd, vtkPoints* pts, vtkCellArray* lines, vtkCellArray* verts, int color_index)
  {
    if (!geom)
    {
      return 0;
    }
	//std::cout << " insertGeometryRecursive {" << std::endl;
    ...
    vtkIdType nCells = 0;
    vtkIdType point_id;
    float color = color_index;
    vtkDoubleArray* pscalar = vtkArrayDownCast<vtkDoubleArray>(pd->GetCellData()->GetScalars());
    ...
    switch (geom->getGeometryType())
    {
      ...
      case wkbPoint25D:
        gpt = (OGRPoint*)geom;
        point_id = pts->InsertNextPoint(gpt->getX(), gpt->getY(), gpt->getZ());
        ptIds.push_back(point_id);
        pscalar->InsertNextValue(color);
      ...
      case wkbLineString25D:
        ...
        {
        pscalar->InsertNextValue(color);
          point_id = pts->InsertNextPoint(gls->getX(p), gls->getY(p), gls->getZ(p));
          // insert ring points
          ptIds.push_back(point_id);
        }
      ...
      case wkbPolygon25D:
        ...
        nCells += this->insertGeometryRecursive(grng, pd, pts, lines, verts, color_index);
      ...
      case wkbGeometryCollection25D:
        ...
        for (i = 0; i < num; ++i)
        {
          nCells += this->insertGeometryRecursive(gcol->getGeometryRef(i), pd, pts, lines, verts, color_index);
        }
        break;
