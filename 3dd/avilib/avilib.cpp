...
static size_t avi_read(int fd, char *buf, size_t len)
{
    ...
    while (r < len)
    {
        n = read(fd, buf + r, (unsigned int)(len - r));
...
static size_t avi_write(int fd, const char *buf, size_t len)
{
    size_t n = 0;
    size_t r = 0;
    ...
    while (r < len)
    {
        n = write(fd, buf + r, (unsigned int)(len - r));
        if ((ssize_t)n < 0)
            return n;
...
avi_t *AVI_open_output_file(const char *filename)
{
    ...
    for (i = 0; i < HEADERBYTES; i++)
        AVI_header[i] = 0;
    i = avi_write(AVI->fdes, (const char *)AVI_header, HEADERBYTES);
    if (i != HEADERBYTES)
    {
        close(AVI->fdes);
        AVI_errno = AVI_ERR_WRITE;
...
static int avi_write_data(avi_t *AVI, const char *data, unsigned long length, int audio, int keyframe)
{
    int n;
...
void AVI_print_error(char *str)
{
    ...
    if (AVI_errno == AVI_ERR_OPEN || AVI_errno == AVI_ERR_READ || AVI_errno == AVI_ERR_WRITE || AVI_errno == AVI_ERR_WRITE_INDEX ||
        AVI_errno == AVI_ERR_CLOSE)
    {
        perror("REASON");
    }
