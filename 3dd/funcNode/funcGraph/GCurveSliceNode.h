...
class GCurveSliceNode : public GraphNode
{
public:
	GCurveSliceNode(std::shared_ptr< XmlProject> project);
	~GCurveSliceNode();
public:
	static void getDefault(SuperNodeStruct*);		// 取默认参数
	bool setParameter(std::shared_ptr<SuperNodeStruct> sliceMap);
	bool getParameter(G_Curve_Slice &sliceMap);
...
private:
	...
	vtkSmartPointer<vtkLoopSubdivisionFilter> subDiv=nullptr;
