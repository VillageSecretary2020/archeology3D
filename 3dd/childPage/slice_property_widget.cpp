...
slice_property_widget::slice_property_widget(int num, SuperNodeStruct* param)
{
	QString label = (param) ? QString::fromStdString(param->m_name) : ( (num == 0)? STRING_LABEL_SLICE : STRING_LABEL_SLICE + QString::number(num + 1));
	...
	m_ori_pos_treeitem = new QTreeWidgetItem(m_normal_property, QStringList(STRING_ITEM_ORI_DAT_SLICE));
	m_ori_checked_treeitem=new QTreeWidgetItem(m_ori_pos_treeitem, QStringList(STRING_ITEM_CHECK_POS));
	m_ori_pos_chk = new QCheckBox;
	m_ori_pos_chk->setChecked(false);
	m_normal_property->setItemWidget(m_ori_checked_treeitem, 1, m_ori_pos_chk);
	m_ori_pos_treeitem->setHidden(true);
	...
	m_return_slice_property_struct = new G_SliceMap;
	baseStruct = m_return_slice_property_struct;
	
	if (!param) {
		GSliceMapNode::getDefault(baseStruct);
	}
	else
	{
		*m_return_slice_property_struct = *(G_SliceMap *)param;
	}
		G_SliceMap *slice= m_return_slice_property_struct;
	{
		m_input_treeitem->setText(1, QString::fromStdString(slice->m_putIn));
		m_ori_pos_chk->setChecked(slice->m_original_data_slice);
...
*/
	...
	this->NodeType = STRING_GTYPE_SLICE;
	m_can_append_widget.push_back(STRING_WIDGET_AXIS);
	m_can_append_widget.push_back(STRING_WIDGET_BORDER);
	// yzw 0707 赋值之后再connect，否则引发m_update_widget_property崩溃
	connect(m_ori_pos_chk, SIGNAL(stateChanged(int)), this, SLOT(slot_lighting()));
...
slice_property_widget::~slice_property_widget()
{
	delete m_form1_tab;
}

void slice_property_widget::setCutPlaneRange(float* dataRange)
{
		m_cut_plane->set_range(dataRange);
}

void slice_property_widget::setCutPoint(double* dataRange)
{
	ia_tmr.stop();
	ia_tmr.start();
...
void slice_property_widget::paintEvent(QPaintEvent *e)
{
#if 0
	QPainter p(this);
...
SuperNodeStruct* slice_property_widget::return_slice_property_struct()
{
	getBaseStruct();
	...
	m_return_slice_property_struct->m_original_data_slice = m_ori_pos_chk->isChecked();
...
void slice_property_widget::contextMenuEvent(QContextMenuEvent *event)
{
...
void slice_property_widget::delete_self()
{
	emit sig_delete_slice();
}

void slice_property_widget::m_update_widget_property()
{
	G_SliceMap* pTemp = (G_SliceMap*)this->return_slice_property_struct();
...
void slice_property_widget::slot_slice_to_axis()
{
	emit sig_slice_to_axis();
}

void slice_property_widget::slot_slice_to_border()
{
	emit sig_slice_to_border();
}

void slice_property_widget::slot_lighting()
{
	m_update_widget_property();
}

void slice_property_widget::m_slider_setValue(double a)
{
...
void slice_property_widget::m_spin_setValue(int a)
{
	//if (!m_sliderPressed)
	{
		baseStruct->m_changetype = 1;
...
void slice_property_widget::slot_slider_pressed()
{
	m_sliderPressed = true;
}

void slice_property_widget::slot_slider_relesed()
{
	m_sliderPressed = false;
...
void slice_property_widget::slot_color_method()
{
	m_update_widget_property();
}

void slice_property_widget::slot_place_direction()
{
	baseStruct->m_changetype = 4;
	m_update_widget_property();
}

void slice_property_widget::slot_x_normal()
{
	baseStruct->m_changetype = 4;
	m_update_widget_property();
}

void slice_property_widget::slot_y_normal()
{
	baseStruct->m_changetype = 4;
	m_update_widget_property();
}

void slice_property_widget::slot_z_normal()
{
	baseStruct->m_changetype = 4;
	m_update_widget_property();
}

void slice_property_widget::slot_offset_from_center_slider()
{
	baseStruct->m_changetype = 4;
...
void slice_property_widget::slot_offset_from_center_spin(int a)
{
	if (!m_sliderPressed)
	{
		baseStruct->m_changetype = 4;
...
void slice_property_widget::slot_offset_center_slider_pressed()
{
	m_sliderPressed = true;
}

void slice_property_widget::slot_offset_center_slider_relesed()
{
	m_sliderPressed = false;
...
void slice_property_widget::slot_cut_plane_property_changed(Child_CutingPlane*, int type)
{
	baseStruct->m_changetype = G_Slice_MN_CT + type;
	m_update_widget_property();
}

void slice_property_widget::slot_cut_point_timeout()
{
	ia_tmr.stop();
	baseStruct->m_changetype = G_Slice_MN_CT + 40;
	m_update_widget_property();
	//baseStruct->m_changetype = 0;
}

void slice_property_widget::slot_show_color_scale(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_place_direction(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_m_x_position_slider(double a)
{
...
void slice_property_widget::slot_m_x_position_spin(int a)
{
...
void slice_property_widget::slot_m_y_position_slider(double a)
{
...
void slice_property_widget::slot_m_y_position_spin(int a)
{
...
void slice_property_widget::slot_m_width_slider(double a)
{
...
void slice_property_widget::slot_m_width_spin(int a)
{
...
void slice_property_widget::slot_m_length_slider(double a)
{
...
void slice_property_widget::slot_m_length_spin(int a)
{
...
void slice_property_widget::slot_m_font_size_slider(int a)
{
...
void slice_property_widget::slot_m_font_size_spin(int a)
{
...
void slice_property_widget::slot_m_note_number(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_m_note_font_size_slider(int a)
{
...
void slice_property_widget::slot_m_note_font_size_spin(int a)
{
...
void slice_property_widget::slot_m_adopt_self_note(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_color_type(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_significant(int a)
{
	m_update_widget_property();
}

void slice_property_widget::slot_color_scale_property_changed(Child_ColorBar*ccb, int type)
{
	if (0 == type)
		m_update_widget_property();
/*	else if (1 == type){
