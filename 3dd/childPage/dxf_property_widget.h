...
class dxf_property_widget:public base_property_widget
{
	Q_OBJECT
public:
	dxf_property_widget(int num, QString filename, G_Picture* bound=NULL);
	~dxf_property_widget();
	G_Picture* return_picture_form_property_struct();
	virtual void delete_self();
	void set_xlength_ywidth(double, double);
private:
...
private:	
	...
	QSlider* m_pic_scale;
	QLineEdit *m_pic_scale_edit, *m_y_scale_edit, *m_z_scale_edit;
	bool m_scalePressed;
	...
	G_Picture* m_return_picture_property_struct;
private:
