...
geotiff_property_widget::geotiff_property_widget(int num, QString filename, G_Geotiff *data)
{
	m_return_picture_property_struct = new G_Geotiff();
	baseStruct = m_return_picture_property_struct;
	QString label = (data) ? QString::fromStdString(data->m_name) : ( (num == 0)? STRING_LABEL_GEOTIFF : STRING_LABEL_GEOTIFF + QString::number(num + 1));
...
//	m_Normal_TreeWidget->setItemWidget(m_textureFile_treeitem, 1, m_textureFile_edit);
	...
	m_pic_opacity_treeitem = new QTreeWidgetItem(m_normal_property_treeitem, QStringList(QString(STRING_ITEM_OPACITY)));
	m_opacity_slider = new QSlider(Qt::Horizontal);
	m_opacity_slider->setRange(0, 1000 - 1);
	m_opacity_slider->setValue(800);
	m_Normal_TreeWidget->setItemWidget(m_pic_opacity_treeitem, 1, m_opacity_slider);
	...
	m_probe_pos_treeitem = new QTreeWidgetItem(m_normal_property_treeitem, QStringList(QString(STRING_ITEM_PROBE_POS)));
	m_probe_pos_cb = new QCheckBox;
	m_probe_pos_cb->setText(STRING_ITEM_CHECK_POS);
	m_probe_pos_cb->setEnabled(false);
	m_label_pos = new QLabel;
	QHBoxLayout *probe_pos_layou = new QHBoxLayout;
	probe_pos_layou->addWidget(m_probe_pos_cb, 30);
	probe_pos_layou->addWidget(m_label_pos, 70);
	QWidget *probe_pos_widget = new QWidget;
	probe_pos_widget->setLayout(probe_pos_layou);
	m_Normal_TreeWidget->setItemWidget(m_probe_pos_treeitem, 1, probe_pos_widget);
	...
	m_reset_button = new QPushButton;	m_reset_button->setText(STRING_ITEM_PIC_RESET);
	 ...
	 m_adopt_combo->insertItem(0, STRING_ITEM_PIC_TIFF_POS);
	 m_adopt_combo->insertItem(1, STRING_ITEM_PIC_ADJUST_POS);	//弃用STRING_ITEM_PIC_RECT_SEL
	...
	if (!data) {
		GGeotiffNode::getDefault(baseStruct);
		data = (G_Geotiff*)baseStruct;
	}
	{
		...
		m_opacity_slider->setValue(data->xy_range[5]);
	...
	connect(m_opacity_slider, SIGNAL(valueChanged(int)), this, SLOT(slot_opacity_changed(int)));
	connect(m_probe_pos_cb, SIGNAL(stateChanged(int)), this, SLOT(slot_drag_enable(int)));
...
geotiff_property_widget::~geotiff_property_widget()
{
	delete m_form1_tab;
}

void geotiff_property_widget::paintEvent(QPaintEvent *e)
{
...
void geotiff_property_widget::contextMenuEvent(QContextMenuEvent  *event){
...
void geotiff_property_widget::delete_self()
{
	emit sig_delete_geotiff();
}

G_Geotiff* geotiff_property_widget::return_picture_form_property_struct()
{
	getBaseStruct();
	m_return_picture_property_struct->picFilePath = m_textureFile_edit->text().toStdString();
	m_return_picture_property_struct->adopt = m_adopt_combo->currentIndex() == 0 ? 0 : 1;
	...
	m_return_picture_property_struct->xy_range[5] = m_opacity_slider->value();
...
void geotiff_property_widget::m_update_widget_property()
{
	G_Geotiff* pTemp = this->return_picture_form_property_struct();
...
void geotiff_property_widget::picture_change(int type)
{
	baseStruct->m_changetype = G_Picture_MN_CT + type;
	m_update_widget_property();
}

void geotiff_property_widget::slot_opacity_changed(int)
{
	baseStruct->m_changetype = G_Picture_MN_CT + 42;
	m_update_widget_property();
}

void geotiff_property_widget::slot_scale_edit()
{
...
void geotiff_property_widget::slot_scale_slider()
{
...
void geotiff_property_widget::slot_pic_scale_pressed()
{
	m_scalePressed= true;
}

void geotiff_property_widget::slot_pic_scale_relesed()
{
	m_scalePressed= false;
	picture_change(0xf0);
}

void geotiff_property_widget::slot_adopt_combo()
{
	bool adj = m_adopt_combo->currentIndex() == 0;
	//m_pic_adjust_pos_treeitem->setHidden(!adj);
	if(adj)
		m_probe_pos_cb->setChecked(false);
	m_probe_pos_cb->setEnabled(!adj);
	if(0)
	{
		m_pic_x_offset->setHidden(!adj);
		m_pic_y_offset->setHidden(!adj);
		m_pic_z_offset->setHidden(!adj);
	}
	//m_pic_rect_sel_treeitem->setHidden(adj);
	picture_change(20 + (adj ? 0 : 1)); 
}

void geotiff_property_widget::slot_pic_apply(int s)
{
	bool ap = s;
	...
	if(ap){
		m_probe_pos_cb->setChecked(!ap);
		m_probe_pos_cb->setDisabled(true);
	}else if(m_adopt_combo->currentIndex() >0)
		m_probe_pos_cb->setEnabled(true);
	picture_change(0x80);
}

void geotiff_property_widget::slot_x_offset_edit()
{
...
void geotiff_property_widget::slot_y_offset_edit()
{
...
void geotiff_property_widget::slot_z_offset_edit()
{
...
void geotiff_property_widget::slot_z_rotate_edit()
{
...
void geotiff_property_widget::slot_x_offset_slider()
{
...
void geotiff_property_widget::slot_y_offset_slider()
{
...
void geotiff_property_widget::slot_z_offset_slider()
{
...
void geotiff_property_widget::slot_z_rotate_slider()
{
...
void geotiff_property_widget::slot_reset_offset()
{
	float ftmp = 0.0;
...
void geotiff_property_widget::slot_select_file()
{
...
void geotiff_property_widget::slot_drag_enable(int ch)
{
	picture_change(40 +(0< ch ?1:0));
}

void geotiff_property_widget::select_point(int ch)
{
	int ic;
	int type = 22;
...
void geotiff_property_widget::slot_select_point1()
{
	select_point(m_select_point[0]->isChecked() ? 1 : -1);
}

void geotiff_property_widget::slot_select_point2()
{
	select_point(m_select_point[1]->isChecked() ? 2 : -2);
}

void geotiff_property_widget::slot_select_point3()
{
	select_point(m_select_point[2]->isChecked() ? 3 : -3);
}

void geotiff_property_widget::curPos(double* pos)
{
	//m_label_pos->setText(QString("%1f %2f ").arg(pos[0]).arg(pos[1]));
	m_label_pos->setText(QString::number(pos[0],'f') +", " +QString::number(pos[1],'f'));
}