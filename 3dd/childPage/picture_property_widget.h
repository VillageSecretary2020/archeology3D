...
class picture_property_widget:public base_property_widget
{
	Q_OBJECT
public:
	picture_property_widget(int num, QString filename, G_Picture* bound=NULL);
	~picture_property_widget();
...
signals:
	...
	void sig_delete_picture();
public slots:
private slots:
	void m_update_widget_property();
	void slot_drag_enable();
